...
import subprocess, yaml, sys, os, time, json, threading, shutil
from training import train_config
from datetime import datetime
from threading import Thread
from training.val import run_val
...
DEFAULT_HYP = ROOT_DIR / "yolov9" / "data" / "hyps" / "hyp.scratch-high.yaml"
TRAIN_SCRIPT = ROOT_DIR / "yolov9" / "train_dual.py"
YOLO_TRAIN = ROOT_DIR / "yolov9" / "train.py"
DOWNLOAD_ROOT = Path("/shared") / "download"
EXPORT_SCRIPT = ROOT_DIR / "deployment" / "export.py"
...
def run_training(dataset_name: str, 
    ...
    if not data_yaml_path:
        ...
        data_yaml_path = str(ROOT_DIR / "Dataset" / data_yaml_rel / "data.yaml")
...
def train_model(
    ...
    base_home_dir    = str(Path(ROOT_DIR, 'dataset_home', *dataset_name.split('/')))
    base_dataset_dir = str(Path(ROOT_DIR, 'Dataset', *dataset_name.split('/')))
    if vname:
...
def delete_training_project(
    ...
    # 刪除 download 對應
    dl_dir = str(DOWNLOAD_ROOT / user_n / project)
    _safe_rmtree(dl_dir)
...
def delete_training_task(
    ...
    # 刪除 download 對應
    dl_dir = str(DOWNLOAD_ROOT / user_n / project / task)
    _safe_rmtree(dl_dir)
...
def delete_training_version(
    ...
    # 刪除 download 對應
    dl_dir = str(DOWNLOAD_ROOT / user_n / project / task / version)
    _safe_rmtree(dl_dir)
